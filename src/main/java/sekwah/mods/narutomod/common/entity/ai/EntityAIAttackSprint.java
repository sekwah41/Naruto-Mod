...
public class EntityAIAttackSprint extends EntityAIBase
{
    World worldObj;
    ...
    public EntityAIAttackSprint(EntityCreature p_i1635_1_, Class p_i1635_2_, double p_i1635_3_, boolean p_i1635_5_)
    {
        this(p_i1635_1_, p_i1635_3_, p_i1635_5_);
        this.classTarget = p_i1635_2_;
    }

    public EntityAIAttackSprint(EntityCreature p_i1636_1_, double p_i1636_2_, boolean p_i1636_4_)
    {
    ...
    private void setSprinting(boolean isSprinting) {
        this.attacker.setSprinting(isSprinting);
        this.attacker.getNavigator().setSpeed(isSprinting ? this.speedTowardsTarget * 1.1f : this.speedTowardsTarget);
    }
    /**
     * Resets the task
     */
    ...
    public void updateTask()
    {
        if(this.sprintCooldown-- <= 0) {
            if(!this.attacker.isSprinting()) {
                this.setSprinting(true);
            }
        }
        else if(this.attacker.isSprinting()) {
            this.setSprinting(false);
        }
        EntityLivingBase entitylivingbase = this.attacker.getAttackTarget();
        ...
        if ((this.longMemory || this.attacker.getEntitySenses().canSee(entitylivingbase)) && this.field_75445_i <= 0 && (this.field_151497_i == 0.0D && this.field_151495_j == 0.0D && this.field_151496_k == 0.0D || entitylivingbase.getDistanceSq(this.field_151497_i, this.field_151495_j, this.field_151496_k) >= 0.0D || this.attacker.getRNG().nextFloat() < 0.05F))
        {
            ...
            this.field_75445_i = 4 + this.attacker.getRNG().nextInt(14);
        ...
        {
            this.attackTick = 15 + rand.nextInt(10);
            ...
            sprintCooldown = 40 + rand.nextInt(20);
