...
public class EntityAINinjaPlay extends EntityAIBase {
    private EntityNinjaVillager villagerObj;
    private EntityLiving targetVillager;
    private float field_75261_c;
    private int playTime;

    public EntityAINinjaPlay(EntityNinjaVillager par1EntityNinjaVillager, float par2) {
    ...
    public boolean shouldExecute() {
        if (this.villagerObj.getGrowingAge() >= 0) {
            ...
            List var1 = this.villagerObj.worldObj.getEntitiesWithinAABB(EntityNinjaVillager.class, this.villagerObj.boundingBox.expand(6.0D, 3.0D, 6.0D));
            double var2 = Double.MAX_VALUE;
            Iterator var4 = var1.iterator();

            while (var4.hasNext()) {
                EntityNinjaVillager var5 = (EntityNinjaVillager) var4.next();
